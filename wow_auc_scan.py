...
INFO_URL="http://eu.battle.net/auction-data/ecee6e0f05b6c1c05d9e1c1822a5c11b/auctions.json"
INFO_URL2="http://eu.battle.net/auction-data/202821d1c5cd21159ede2548cc838c23/auctions.json"
auc_list_a=[]
auc_list_b=[]
compare=[]
compare_2=[] #temporary list
result=[]
realm_a=None
realm_b=None


def get_data(auc_list,auc_api,order):
	...
	for line in content:
		...
		#print(type(y))
		auc_list.append(y)
		#auction = json.loads(content)
		'''with open('auction.txt', 'w') as correct:
			for line in auc_list:
				correct.write(str(line) + '\n')'''
		
	auc_list=list(auc_list[3:-1])
	check_auctions(auc_list,order)

def main():
	get_data(auc_list_a,INFO_URL,1)
	get_data(auc_list_b,INFO_URL2,2)
	filtering()

def check_auctions(realm_auction_list, order):
	for auction in realm_auction_list:
		for details in auction:
			if 'item:82800' in details:
				if order == 2:
					compare.append(auction)
				else:
					compare_2.append(auction)


def filtering():
	for auction in compare:
		for comparison_auction in compare_2:
			if auction[-5:-1]==comparison_auction[-5:-1]:
				print(' Auction 1: ',str(name_check(str(auction[-5]))), auction[2:4],str((auction[5])[:-4]), auction[-4:-1], '\n Similar to\n Auction 2: ',str(name_check(str(comparison_auction[-5]))),comparison_auction[2:4], str((comparison_auction[5])[:-4]),comparison_auction[-4:-1], '\n\n')


# TODO incorporate pets in this format 
#'''{"auc":1879400563,"item":82800,"owner":"tester","ownerRealm":"Darksorrow","bid":28500000,"buyout":30000000,"quantity":1,"timeLeft":"LONG","rand":0,"seed":979244928,"context":0,"modifiers":[{"type":3,"value":1237},{"type":4,"value":50331656},{"type":5,"value":1}],"petSpeciesId":1237,"petBreedId":8,"petLevel":1,"petQualityId":3},
...
def name_check(id):
	...
	'MoonkinHatchling':'petSpeciesId:296',
